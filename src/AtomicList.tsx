...
import { Palette, Save, Download, Upload, Trash2, AlertTriangle, ChevronLeft, ChevronRight, Search, FileText, X, Link, ExternalLink, Info, Circle, CheckCircle2, ListTodo } from 'lucide-react';
...
const AtomicListApp = () => {
  ...
  const [linkNodeId, setLinkNodeId] = useState<number | null>(null);
      ...
      if (navigationMode && nodes.length > 0) {
        ...
        switch (e.key) {
          ...
          case 't':
            e.preventDefault();
            if (activeNode) {
              if (activeNodeId !== null) handleToggleTask(activeNodeId);
            }
            break;
            
          case 'f':
            e.preventDefault();
            if (activeNodeId !== null) handleNodeFocus(activeNodeId);
            break;
            
          case 'x':
            e.preventDefault();
            if (activeNodeId !== null) handleNodeLink(activeNodeId);
            break;
          ...
          case 'd':
            ...
            if (activeNodeId !== null) handleNodeDelete(activeNodeId);
...
<body>
  ...
  const handleNodeSwipe = (nodeId: number, direction: string) => {
    if (direction === 'right') {
      // Swipe right = indent
      setNodes(prev => prev.map(node => 
        node.id === nodeId 
  ...
  const handleTouchStart = (e: React.TouchEvent, nodeId: number) => {
    ...
    (e.currentTarget as any).nodeId = nodeId;
    
    // Set up long press timer
    (e.currentTarget as any).longPressTimer = setTimeout(() => {
      // Long press detected - select node
      handleNodeClick(nodeId);
      (e.currentTarget as any).isLongPress = true;
    }, 500); // 500ms for long press
  };

  const handleTouchMove = (e: React.TouchEvent) => {
    // If user moves too much during touch, cancel long press
    if (!(e.currentTarget as any).touchStartX || !(e.currentTarget as any).touchStartY) return;
    
    const touch = e.touches[0];
    const deltaX = Math.abs(touch.clientX - (e.currentTarget as any).touchStartX);
    const deltaY = Math.abs(touch.clientY - (e.currentTarget as any).touchStartY);
  ...
  const handleTouchEnd = (e: React.TouchEvent) => {
    ...
    if ((e.currentTarget as any).isLongPress) {
      (e.currentTarget as any).isLongPress = false;
      // Clean up
      delete (e.currentTarget as any).touchStartX;
      delete (e.currentTarget as any).touchStartY;
      delete (e.currentTarget as any).touchStartTime;
      delete (e.currentTarget as any).nodeId;
      return;
    }
    ...
    const nodeId = (e.currentTarget as any).nodeId;
    const touchDuration = Date.now() - (e.currentTarget as any).touchStartTime;
  ...
  const handleNodeLink = (nodeId: number) => {
      ...
      <div className="flex flex-col h-screen">
        ...
        <div 
          ...
          onClick={(e) => {
            ...
            if (e.target === e.currentTarget || (e.target as HTMLElement).closest('.node-container') === null) {
              setSelectedNodeId(null);
              setActiveNodeId(null);
              setNavigationMode(false);
            }
